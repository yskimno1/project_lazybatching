...
double input_time[] = {1, 10, 15, 16, 17};
enum layer_t model_info[] = {CONV, CONV, CONV, CONV, CONV_LAST};
...
struct Model_layer changeModel(struct Model_layer model, int index, cudnnHandle_t cudnnHandle, cudaStream_t myStream_compute, cudaStream_t myStream_mem,
    ...
    model.list_layer[MAX_LAYER_NUM-1] = new Layer(CONV_LAST, &cudnnHandle, &myStream_compute, &myStream_mem, 0, n_in, c_in, h_in, w_in, pad_h, pad_w, stride_h, stride_w, k, r, s, n_out, c_out, h_out, w_out);
    model.list_layer[MAX_LAYER_NUM-2]->setDstData(model.list_layer[MAX_LAYER_NUM-1]->SrcData());
